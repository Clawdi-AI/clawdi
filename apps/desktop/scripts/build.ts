...
const webPublicRoot = join(webRoot, "dist", "client");
...
await buildWebApp();
cpSync(webPublicRoot, packagedWebRoot, { recursive: true });
writeFileSync(
...
async function buildWebApp(): Promise<void> {
	const webBuild = Bun.spawn(["bun", "run", "build"], {
		...
		env: {
			...
			VITE_CLERK_PUBLISHABLE_KEY:
				globalThis.process.env.VITE_CLERK_PUBLISHABLE_KEY?.trim() ||
	...
	if (!existsSync(join(webPublicRoot, "index.html"))) {
