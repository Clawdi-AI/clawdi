...
describe("connector auth flow classification", () => {
	test.each(["oauth", "oauth1", "oauth2", "OAUTH2", "dcr_oauth", "composio_link"])(
		"routes %s through the redirect flow",
		(authType) => {
			expect(getConnectorAuthFlow(authType)).toBe("redirect");
		},
	);

	test.each(["api_key", "API_KEY", "bearer_token", "BEARER_TOKEN", "basic", "BASIC"])(
		"routes %s through the credentials flow",
		(authType) => {
	...
	test.each([undefined, null, "", "   ", "unknown", "session_token"])(
		"rejects unsupported auth_type value %p instead of guessing",
		(authType) => {
