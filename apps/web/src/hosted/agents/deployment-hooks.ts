...
export function useAgentDeployment(environmentId: string, deploymentSelector?: string | null) {
	...
	// env-id route it's the route param itself; for a canonical Hosted route,
	// resolve the deployment id to the stored cloud-api env id, falling back to
	// the route param while deployment creation has not projected an env id yet.
