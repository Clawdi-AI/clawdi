...
import { tarSingleFile, tarSkillDir } from "../lib/tar-helpers";
...
export async function skillsList() {
	...
	const skills = await api.get<SkillRecord[]>("/api/skills");
	...
	for (const s of skills) {
		...
		console.log(
			`  ${chalk.white(s.skill_key)}  v${s.version}  ${chalk.gray(s.source)}${repo}${files}`,
		);
	}
...
export async function skillsAdd(path: string) {
	...
	console.log(
		chalk.green(`✓ Uploaded ${result.skill_key} (v${result.version}, ${result.file_count} files)`),
	);
}
	...
	if (adapters.length > 0) {
		try {
			...
			for (const adapter of adapters) {
				...
				console.log(
					chalk.green(
						`  ✓ ${AGENT_LABELS[adapter.agentType]} → ${skillDir}/ (${installResult.file_count} files)`,
					),
				);
			}
		} catch (e: unknown) {
			const message = e instanceof Error ? e.message : String(e);
			console.log(chalk.yellow(`  ⚠ Local install failed: ${message}`));
		}
	}
