...
import { chmodSync, existsSync, lstatSync, mkdtempSync, readdirSync, readFileSync, rmSync } from "node:fs";
...
export interface HostedHermesSkillExactSourceDriver {
	install(input: { home: string; appRoot: string; skill: PreparedHostedCatalogSkill; previouslyReserved: boolean }): "installed" | "unchanged";
...
function commandPath(home: string, appRoot: string): string {
	for (const candidate of [join(appRoot, "venv", "bin", "hermes"), join(home, ".local", "bin", "hermes")]) {
		if (executableExists(candidate)) return candidate;
	}
	throw new Error("installed Hermes Skill CLI is unavailable");
}

function targetDir(home: string, skillId: string): string { return join(home, ".hermes", "skills", skillId); }
function receiptPath(home: string, skillId: string): string { return join(home, ".hermes", "skills", ".clawdi-manifest-receipts", `${skillId}.json`); }

function writeReceipt(home: string, skill: PreparedHostedCatalogSkill, target: string): void {
	writePrivateFileAtomic(receiptPath(home, skill.skillId), `${JSON.stringify({ schemaVersion: "clawdi.hermesManifestSkillReceipt.v1", skillId: skill.skillId, archiveSha256: skill.digest, fingerprint: fingerprint(boundedFiles(target)) })}\n`, { mode: 0o600, dirMode: 0o700 });
}
...
function fingerprint(files: Map<string, Buffer> | null): string | null {
	...
	for (const [name, bytes] of [...files].sort(([left], [right]) => left.localeCompare(right))) hash.update(name).update("\0").update(bytes).update("\0");
	return hash.digest("hex");
}

function withStagedSkill<T>(skill: PreparedHostedCatalogSkill, operation: (sourceDir: string) => T): T {
	const root = mkdtempSync(join(tmpdir(), "clawdi-hermes-skill-"));
	try {
		const result = spawnSync("tar", ["-xzf", "-", "-C", root], { input: skill.tarBytes, stdio: ["pipe", "pipe", "pipe"], maxBuffer: 1024 * 1024 });
...
function rawSkillUrl(skill: PreparedHostedCatalogSkill): string {
	...
	if (!owner || !repo) throw new Error("Hermes exact-source URL requires a canonical GitHub repository");
...
function runHermes(input: { home: string; appRoot: string }, args: string[]) {
	return spawnRuntimeUserCommand(commandPath(input.home, input.appRoot), args, input.home, input.appRoot, {
		hermesHome: join(input.home, ".hermes"), timeoutMs: 120_000, maxBufferBytes: 1024 * 1024,
	});
}
...
export const hostedHermesSkillExactSourceDriver: HostedHermesSkillExactSourceDriver = {
	install(input) {
		return withStagedSkill(input.skill, (sourceDir) => {
			...
			if (existsSync(target) && !input.previouslyReserved) throw new Error("Hermes Skill target is not paired with a manifest reservation");
			if (sameFiles(boundedFiles(sourceDir), boundedFiles(target))) {
				writeReceipt(input.home, input.skill, target);
				return "unchanged";
			}
			const result = runHermes(input, ["skills", "install", rawSkillUrl(input.skill), "--name", input.skill.skillId, "--yes"]);
			if (result.status !== 0) throw new Error(`Hermes official Skill install failed: ${String(result.stderr || result.stdout).trim() || "unknown error"}`);
			if (!sameFiles(boundedFiles(sourceDir), boundedFiles(target))) {
				runHermes(input, ["skills", "uninstall", input.skill.skillId, "--yes"]);
				throw new Error("Hermes URL install cannot losslessly express the exact catalog archive");
			}
			writeReceipt(input.home, input.skill, target);
			return "installed";
		});
	},
	verifyOwned(input) { return withStagedSkill(input.skill, (sourceDir) => sameFiles(boundedFiles(sourceDir), boundedFiles(targetDir(input.home, input.skill.skillId)))); },
	uninstall(input) {
		const target = targetDir(input.home, input.skillId);
		if (!existsSync(target)) return "absent";
		let receipt: unknown;
		try { receipt = JSON.parse(readFileSync(receiptPath(input.home, input.skillId), "utf8")); } catch { receipt = null; }
		const value = receipt && typeof receipt === "object" && !Array.isArray(receipt) ? receipt as Record<string, unknown> : null;
		if (value?.schemaVersion !== "clawdi.hermesManifestSkillReceipt.v1" || value.skillId !== input.skillId || value.fingerprint !== fingerprint(boundedFiles(target))) throw new Error("Hermes Skill bytes no longer match the manifest ownership receipt");
		const result = runHermes(input, ["skills", "uninstall", input.skillId, "--yes"]);
		if (result.status !== 0) throw new Error(`Hermes official Skill uninstall failed: ${String(result.stderr || result.stdout).trim() || "unknown error"}`);
