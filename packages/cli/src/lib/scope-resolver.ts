...
/**
 * Resolve a user-supplied `<scope>` argument to a backend scope UUID.
 *
 * Accepts:
 *   - A full UUID → returned as-is (no round-trip).
 *   - A slug (matches `Scope.slug`) → resolved via GET /api/scopes.
 *   - A human name → matched against `Scope.name` (case-insensitive)
 *     after slug match fails.
 *   - `default` or omitted → returns the user's default-write scope.
 *
...
export async function resolveScopeId(
	...
	const scopes = await authedGet<ScopeBrief[]>(apiUrl, bearer, "/api/scopes");
	...
	if (matches.length === 0) {
		throw new Error(`No scope matches '${input}'. Try \`clawdi scope list\` to see your scopes.`);
	}
	if (matches.length > 1) {
