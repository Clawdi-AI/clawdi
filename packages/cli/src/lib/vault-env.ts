...
export function renderVaultEnv(content: string, apiUrl: string, material: VaultMaterial): string {
...
function requireUntrackedIgnored(path: string): void {
	...
	const ignored = spawnSync("git", ["check-ignore", "-q", "--", path], { cwd, encoding: "utf8" });
...
export async function updateVaultEnv(
...
): Promise<{ path: string; fields: number }> {
	...
	try {
		...
		return { path, fields: Object.keys(material.values).length };
	} finally {
