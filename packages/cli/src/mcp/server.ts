...
export interface ConnectorToolContext {
	mcpConfig: { mcp_url: string; mcp_token: string } | null;
	remoteTools: McpTool[];
}

export async function loadConnectorToolContext(): Promise<ConnectorToolContext> {
	...
	return { mcpConfig, remoteTools };
}

export async function createClawdiMcpServer(
	...
	const { getConfig } = await import("../lib/config");
	const cliConfig = getConfig();
	const { mcpConfig, remoteTools } = connectorContext ?? (await loadConnectorToolContext());
...
export async function startMcpHttpServer(options: McpHttpServerOptions = {}): Promise<void> {
	...
	// Connector config and remote tool definitions are fetched once at startup;
	// only the (cheap) per-request server/transport pair is rebuilt, as the
	// stateless streamable-http mode requires.
	const connectorContext = await loadConnectorToolContext();
	const httpServer = createServer(async (req, res) => {
		...
		try {
			const mcpServer = await createClawdiMcpServer(connectorContext);
