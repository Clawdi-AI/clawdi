...
describe("adapter Skills paths", () => {
	it("Claude Code: <root>/<key>__<owner-handle>", () => {
		const adapter = new ClaudeCodeAdapter();
		const p = adapter.skills.sharedPath("git-tools", "alice-a3b4");
		expect(p).toMatch(/skills\/git-tools__alice-a3b4$/);
	});

	it("Two owners of same key produce distinct paths", () => {
		const adapter = new ClaudeCodeAdapter();
		const p1 = adapter.skills.sharedPath("git-tools", "alice-a3b4");
		const p2 = adapter.skills.sharedPath("git-tools", "bob-c5d6");
		expect(p1).not.toBe(p2);
	});

	it("Personal skills root stays unchanged", () => {
		const adapter = new ClaudeCodeAdapter();
		const root = adapter.skills.rootDir();
