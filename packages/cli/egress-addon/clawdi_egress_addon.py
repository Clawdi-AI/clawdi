"""Generic Clawdi egress profile interpreter.

The addon intentionally contains no product hostnames or route constants. It
reads all routing policy from a clawdi.egressProfiles.v1 bundle and a sidecar-only
...
SCHEMA_VERSION = "clawdi.egressProfiles.v1"
ENV_FILE_KEY = "CLAWDI_EGRESS_ENV_FILE"
DECISION_METADATA_KEY = "clawdi_egress_decision_applied"
...
class ClawdiEgressAddon:
    ...
    def log_decision(self, flow: Any, decision: ProfileDecision) -> None:
        if decision.profile_id:
            ctx.log.info(
                f"clawdi egress action={decision.action} profile={decision.profile_id} "
    ...
    def reload_from_environment(self, environ: dict[str, str | None]) -> None:
        ...
        bundle = first_nonempty(config.get("CLAWDI_EGRESS_PROFILE_BUNDLE"))
        if not bundle:
            raise ConfigError("CLAWDI_EGRESS_PROFILE_BUNDLE is required")
        secret_file = first_nonempty(config.get("CLAWDI_EGRESS_SECRET_FILE"))
...
def parse_env_file(path: Path) -> dict[str, str]:
    ...
    try:
        ...
        raise ConfigError(f"egress env file is missing: {path}") from exc
    for raw in lines:
        line = raw.strip()
...
def load_profiles(path: Path) -> list[dict[str, Any]]:
    try:
        ...
        raise ConfigError(f"egress profile bundle is missing: {path}") from exc
    if raw.get("schemaVersion") != SCHEMA_VERSION:
        raise ConfigError(f"egress profile bundle must use {SCHEMA_VERSION}")
    profiles = raw.get("profiles")
    if not isinstance(profiles, list):
        raise ConfigError("egress profile bundle profiles must be an array")
    return sorted(
