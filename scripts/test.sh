...
usage() {
	echo "Usage: scripts/test.sh [all|ci|js|cli|desktop|shared|sidecar|web|backend|vault-mcp|provider-recovery-fixture] [suite args...]"
}
...
validate_suite() {
	case "$1" in
		all|backend|ci|js|cli|desktop|shared|sidecar|web|vault-mcp|provider-recovery-fixture)
			;;
		*)
