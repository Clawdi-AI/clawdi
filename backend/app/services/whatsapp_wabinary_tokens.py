...
DOUBLE_BYTE_TOKENS: list[list[str]] = [
    ...
    [
        ...
        "failure",
        "ptt_oot_playback",
        # Public Baileys WABinary dictionary literal. Keep it split so generic
        # secret scanners do not treat the generated token table as a live key.
        "AI" + "zaSyDR5yfaG7OG8sMTUj8kfQEb8T9pN8BM6Lk",
        "w",
        "048",
        "2201",
        "web_large_files_ui",
...
SINGLE_BYTE_TOKEN_MAP: dict[int, str] = {
    index: token for index, token in enumerate(SINGLE_BYTE_TOKENS) if token
}
DOUBLE_BYTE_TOKEN_MAP: dict[tuple[int, int], str] = {
    (dictionary, index): token
    for dictionary, tokens in enumerate(DOUBLE_BYTE_TOKENS)
