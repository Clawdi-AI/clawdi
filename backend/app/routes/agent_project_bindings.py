...
async def list_project_bindings(
    ...
    if (
        not any(row.binding_type == "primary" for row in rows)
        and agent.default_project_id in visible_project_ids
    ):
...
async def delete_project_binding(
    ...
    if binding.binding_type == "primary":
        raise HTTPException(
            ...
            "primary binding cannot be removed; set another primary project first",
        )
