...
async def test_list_skills_etag_binds_revision_and_project(
    ...
    (counter is account-wide, listing is project-scoped). Pre-fix
...
@pytest.mark.asyncio
async def test_project_explicit_upload_targets_named_scope(
    ...
    detail_a = await client.get(f"/api/projects/{env_a.default_project_id}/skills/scoped")
    ...
    detail_b = await client.get(f"/api/projects/{env_b.default_project_id}/skills/scoped")
    assert detail_b.status_code == 404, detail_b.text


@pytest.mark.asyncio
async def test_project_explicit_upload_rejects_other_users_scope(
    client: httpx.AsyncClient, db_session, seed_user
):
