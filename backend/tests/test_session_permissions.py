...
async def test_link_permission_post_is_idempotent(client: httpx.AsyncClient):
    ...
    first = (
        await client.post(f"/api/sessions/{session_id}/permissions", json={"kind": "link"})
    ).json()
...
async def test_link_permission_list_returns_active_row(client: httpx.AsyncClient):
    ...
    listing = (await client.get(f"/api/sessions/{session_id}/permissions")).json()
...
async def test_link_permission_delete_revokes(client: httpx.AsyncClient):
    ...
    r = await client.delete(f"/api/sessions/{session_id}/permissions", params={"kind": "link"})
...
async def test_link_permission_revoke_then_recreate(client: httpx.AsyncClient):
    ...
    await client.delete(f"/api/sessions/{session_id}/permissions", params={"kind": "link"})
...
async def test_permission_create_validates_kind_identifier_consistency(
    ...
    bad = await client.post(f"/api/sessions/{session_id}/permissions", json={"kind": "email"})
    assert bad.status_code == 400

    # user + missing user_id → 400
    bad = await client.post(f"/api/sessions/{session_id}/permissions", json={"kind": "user"})
    assert bad.status_code == 400
...
async def test_permissions_unknown_session_404s(client: httpx.AsyncClient):
    ...
    assert (
        await client.post(f"/api/sessions/{bogus}/permissions", json={"kind": "link"})
    ).status_code == 404
    assert (
        await client.delete(f"/api/sessions/{bogus}/permissions", params={"kind": "link"})
    ).status_code == 404
    assert (await client.get(f"/api/sessions/{bogus}/permissions")).status_code == 404
...
async def test_permissions_reject_cli_apikey_auth(cli_client: httpx.AsyncClient):
    ...
    for method, path, kwargs in [
        ("post", f"/api/sessions/{fake_id}/permissions", {"json": {"kind": "link"}}),
        (
            "delete",
            f"/api/sessions/{fake_id}/permissions",
            {"params": {"kind": "link"}},
        ),
        ("get", f"/api/sessions/{fake_id}/permissions", {}),
    ]:
