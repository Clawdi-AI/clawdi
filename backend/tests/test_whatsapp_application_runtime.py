...
from app.routes.channel_routers.whatsapp_application import _pairing_reply_message_id
...
class _FakeApplicationSidecar:
    def __init__(self, *, fail: bool = False) -> None:
        self.requests: list[WhatsAppApplicationSendRequest] = []
        self.fail = fail

    async def send_application_message(self, request: WhatsAppApplicationSendRequest) -> str:
...
async def _seed_account(
    ...
    *,
    ...
    account = ChannelAccount(
        user_id=user.id,
        provider="whatsapp",
        name=f"application-{uuid4().hex[:12]}",
...
async def test_sidecar_ingress_dedupes_and_routes_group_actor_to_link_inbox(
    ...
    inbox = await client.get(
        f"/v1/channels/whatsapp/application/{account_key}/inbox",
        headers={"Authorization": f"Bearer {tokens[0]}"},
    )
    assert inbox.status_code == 200, inbox.text
    assert inbox.json()["events"] == [
        {
            "sequence": messages[0].inbox_sequence,
            "bindingId": str(binding.id),
            "chatJid": group_jid,
            "actorJid": actor_jid,
            "actorJidAlt": None,
            "messageId": "group-in-1",
            "text": "hello group",
            "pushName": "Participant",
            "timestamp": 1_700_000_000,
        }
...
@pytest.mark.asyncio
async def test_sidecar_ingress_http_rejects_non_application_chat_servers(
    client: httpx.AsyncClient,
    db_session: AsyncSession,
    seed_user: User,
    channel_agent: AgentEnvironment,
...
@pytest.mark.asyncio
async def test_application_inbox_ack_replay_link_isolation_and_outbound_reply(
    client: httpx.AsyncClient,
    db_session: AsyncSession,
    seed_user: User,
    channel_agent: AgentEnvironment,
    ...
    for index, link in enumerate(links):
        chat_jid = f"12036300000000000{index + 2}@g.us"
        actor_jid = f"1555111222{index}@s.whatsapp.net"
        binding = ChannelBinding(
            account_id=account.id,
            ...
            external_chat_type="group",
            external_chat_name=f"Group {index}",
            paired_external_user_id=actor_jid,
        )
        db_session.add(binding)
        ...
        message = ChannelMessage(
            ...
            external_chat_id=chat_jid,
            provider_message_id=f"inbound-{index}",
            provider_event_id=f"message:inbound-{index}",
            provider_event_scope=PROVIDER_EVENT_SCOPE_ACCOUNT,
            text=f"message {index}",
            payload={"actorJid": actor_jid},
        )
        db_session.add(message)
        bindings.append(binding)
        messages.append(message)
    await db_session.commit()
    fake_sidecar = _FakeApplicationSidecar()
    monkeypatch.setattr(
        "app.services.whatsapp_application_runtime.get_configured_whatsapp_sidecar_client",
    ...
    assert [event["messageId"] for event in first.json()["events"]] == ["inbound-0"]
    assert replay.json() == first.json()

    wrong_link_send = await client.post(
        f"/v1/channels/whatsapp/application/{account_key}/messages",
        headers=headers,
        json={
            "bindingId": str(bindings[1].id),
            "clientMessageId": "wrong-link-message",
            "text": "no",
            "final": True,
        },
    )
    sent = await client.post(
        ...
        headers=headers,
        json={
            "bindingId": str(bindings[0].id),
            "clientMessageId": "agent-final-reply-1",
            "text": "  final reply\nsecond line\t ",
            "final": True,
            "replyToSequence": messages[0].inbox_sequence,
        },
    )
    ...
    assert wrong_link_send.status_code == 404
    assert sent.status_code == 200, sent.text
    assert sent_replay.json() == sent.json()
    assert fake_sidecar.requests == [
        WhatsAppApplicationSendRequest(
            jid=bindings[0].external_chat_id,
            text="  final reply\nsecond line\t ",
            message_id="agent-final-reply-1",
            reply_to_message_id="inbound-0",
            reply_to_participant_jid="15551112220@s.whatsapp.net",
        )
    ]

    ack = await client.post(
        f"/v1/channels/whatsapp/application/{account_key}/ack",
        headers=headers,
        json={"throughSequence": messages[0].inbox_sequence},
    )
    ...
    second_link = await client.get(
        f"/v1/channels/whatsapp/application/{account_key}/inbox",
        headers={"Authorization": f"Bearer {tokens[1]}"},
    )
    assert ack.json()["ackedCount"] == 1
    assert repeated_ack.json()["ackedCount"] == 0
    assert empty.json()["events"] == []
    assert [event["messageId"] for event in second_link.json()["events"]] == ["inbound-1"]
    outbound = (
        await db_session.execute(
            select(ChannelMessage).where(
                ChannelMessage.account_id == account.id,
    ...
    assert outbound.provider_message_id == "agent-final-reply-1"
    assert outbound.text == "  final reply\nsecond line\t "
    assert tokens[0] not in json.dumps(outbound.payload)


@pytest.mark.asyncio
async def test_application_outbound_reuses_id_after_metadata_commit_failure(
    client: httpx.AsyncClient,
    db_session: AsyncSession,
    seed_user: User,
    channel_agent: AgentEnvironment,
    ...
    async def fail_first_sent_metadata_commit() -> None:
        nonlocal commit_calls
        commit_calls += 1
        if commit_calls == 2:
            raise RuntimeError("forced metadata commit failure")
        await real_commit()

    monkeypatch.setattr(db_session, "commit", fail_first_sent_metadata_commit)
    account_key = channel_runtime_account_key(account.id)
    headers = {"Authorization": f"Bearer {tokens[0]}"}
    body = {
        "bindingId": str(binding.id),
        "clientMessageId": "stable-after-commit-failure",
        "text": "send exactly once",
        "final": True,
    }

    provider_succeeded = await client.post(
        f"/v1/channels/whatsapp/application/{account_key}/messages",
        headers=headers,
        json=body,
    )
    retry_pending = await client.post(
        f"/v1/channels/whatsapp/application/{account_key}/messages",
        headers=headers,
        json=body,
    )
    replay_sent = await client.post(
        f"/v1/channels/whatsapp/application/{account_key}/messages",
        headers=headers,
        json=body,
    )

    assert provider_succeeded.status_code == 200, provider_succeeded.text
    assert retry_pending.status_code == 200, retry_pending.text
    assert replay_sent.json() == retry_pending.json() == provider_succeeded.json()
    assert [request.message_id for request in fake_sidecar.requests] == [
        "stable-after-commit-failure",
        "stable-after-commit-failure",
    ]
    outbound = (
        await db_session.execute(
            select(ChannelMessage).where(
                ChannelMessage.account_id == account.id,
                ChannelMessage.provider_message_id == "stable-after-commit-failure",
            )
        )
    ).scalar_one()
    assert outbound.payload["deliveryState"] == "sent"


@pytest.mark.asyncio
async def test_application_outbound_has_no_cloud_fallback_when_sidecar_is_unavailable(
    client: httpx.AsyncClient,
    db_session: AsyncSession,
    seed_user: User,
    channel_agent: AgentEnvironment,
    ...
    response = await client.post(
        f"/v1/channels/whatsapp/application/{channel_runtime_account_key(account.id)}/messages",
        headers={"Authorization": f"Bearer {tokens[0]}"},
        json={
            "bindingId": str(binding.id),
            "clientMessageId": "sidecar-required-1",
            "text": "do not send through Cloud",
            "final": True,
        },
    )
    ...
    pending = (
        await db_session.execute(
            select(ChannelMessage).where(
                ChannelMessage.account_id == account.id,
                ChannelMessage.provider_message_id == "sidecar-required-1",
            )
        )
    ).scalar_one()
    assert pending.payload["deliveryState"] == "pending"
    assert pending.payload["transport"] == "baileys_sidecar"
...
async def test_sidecar_pairing_binds_group_jid_to_participant_actor(
    ...
    raw_code = "PAIRAPPLICATIONRUNTIME123"
    ...
    fake_sidecar = _FakeApplicationSidecar(fail=reply_fails)
    monkeypatch.setattr(
