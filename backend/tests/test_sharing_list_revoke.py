...
async def test_list_share_links_returns_prefix_not_raw(client, seed_user, workspace_project):
    ...
    await client.post(f"/v1/projects/{workspace_project.id}/share-links", json={"label": "alpha"})
    await client.post(f"/v1/projects/{workspace_project.id}/share-links", json={"label": "beta"})

    r = await client.get(f"/v1/projects/{workspace_project.id}/share-links")
...
async def test_revoke_share_link_stamps_revoked_at(client, seed_user, workspace_project):
    ...
    seed_user.name = "Alice"
    create = await client.post(f"/v1/projects/{workspace_project.id}/share-links", json={})
    link_id = create.json()["id"]

    r = await client.delete(f"/v1/projects/{workspace_project.id}/share-links/{link_id}")
    ...
    listing = await client.get(f"/v1/projects/{workspace_project.id}/share-links")
...
async def test_revoke_share_link_idempotent(client, seed_user, workspace_project):
    ...
    r1 = await client.delete(f"/v1/projects/{workspace_project.id}/share-links/{link_id}")
    assert r1.status_code == 200
    assert r1.json()["status"] == "revoked"

    r2 = await client.delete(f"/v1/projects/{workspace_project.id}/share-links/{link_id}")
...
async def test_revoke_unknown_link_404(client, seed_user, workspace_project):
    ...
    r = await client.delete(
        f"/v1/projects/{workspace_project.id}/share-links/00000000-0000-0000-0000-000000000000"
    )
...
async def test_revoked_link_blocks_unauth_access(client, seed_user, workspace_project):
    ...
    try:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as anon:
            pre = await anon.get(f"/v1/share/{raw}/preview")
    ...
    try:
        transport = ASGITransport(app=app)
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as anon:
            post = await anon.get(f"/v1/share/{raw}/preview")
