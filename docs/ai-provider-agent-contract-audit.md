...
| OpenClaw | `openclaw@2026.5.12`, `2026.5.18`, `2026.5.27`, `2026.5.28` package smoke | Enabled | `openclaw config patch --stdin` |
...
- `clawdi ai-provider apply --engine codex` writes only
...
- `clawdi ai-provider apply --engine hermes` does a structured merge into
...
- `google_generate_content` direct projection. Hermes has a separate Gemini
  ...
  transport contract.
- `agent_profile` or `oauth_profile` auth except where a future Hermes-specific
  profile contract is pinned.
...
- Docker package smokes passed for `openclaw@2026.5.12`, `2026.5.18`,
  `2026.5.27`, and `2026.5.28` using `openclaw config patch --stdin
  --dry-run --json` with the AI Provider patch shape.

Clawdi behavior:

- `clawdi ai-provider apply --engine openclaw` sends JSON patch content to
