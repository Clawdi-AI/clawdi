...
The Agent page shows status, compute, Projects, Sessions, and resources. Cloud Agents also provide OpenClaw Control UI or Hermes Dashboard, Files, and Terminal. Overview starts with three compact, subtly tinted cards: Chat on Web opens the named runtime dashboard, Chat in Channels opens the Agent's channel management, and AI Providers shows the configured model. Hermes defaults to Chat. The Cloud Agent sidebar orders Overview, the runtime dashboard, Channels, AI Providers, then Sessions. Recent Sessions follows the overview cards, with Compute to its right on desktop and below it on mobile. Compute separates runtime status in its header from plan, hardware, and available subscription facts in its body. The subscription status and eligible Upgrade shortcut share a row, with any renewal or end date below. Upgrade opens the existing Settings plan controls; diagnostics and recovery remain in Settings. Connected Agents show their existing sync Status in the same position, without unsupported chat or channel actions. Workspace follows as a full-width section with two-column resources, then Shared as another full-width section with two-column resources. On mobile, entries stack in the same order. Sessions remains the conversation history.
