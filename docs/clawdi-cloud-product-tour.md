...
The Agent page shows status, compute, Projects, Sessions, and resources. Cloud Agents also provide OpenClaw Control UI or Hermes Dashboard, Files, and Terminal. Overview starts with three compact cards: Web Chat opens the named runtime dashboard, Channels opens the Agent's channel management, and AI Providers shows the configured model. Hermes defaults to Chat. Recent Sessions follows, with Compute to its right on desktop and below it on mobile. Compute separates runtime status in its header from plan, hardware, and available subscription facts in its body. Eligible Included Basic agents have an Upgrade shortcut to the existing Settings plan controls; diagnostics and recovery remain in Settings. Connected Agents show their existing sync Status in the same position, without unsupported chat or channel actions. Workspace follows as a full-width section with two-column resources, then Shared as another full-width section with two-column resources. On mobile, entries stack in the same order. Sessions remains the conversation history.
