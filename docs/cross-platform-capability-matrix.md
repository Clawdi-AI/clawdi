...
| OpenClaw Control UI, Hermes Dashboard, Files, logs, and terminal (`/agents/$id/*`, `/terminal/$id`) | Direct browser surfaces; runtime readiness is distinct from deployment status | Direct renderer surfaces | Adapted touch UI; xterm only in an isolated WebView, never a local terminal | Readiness failures retain the same public category and recovery path; credentials never enter URLs or renderer storage |
...
| Connectors (`/connectors`, detail) | Direct catalog, multi-account aliases, account states, reauthorization, credential updates, enabling, and deletion | Direct with system-browser OAuth handoff | Unsupported initially | Account identity and alias survive recovery; authorization return targets, active-only readiness, and credential redaction match Web |
