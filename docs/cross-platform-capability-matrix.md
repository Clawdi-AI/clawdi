...
| Connectors (`/connectors`, detail) | Direct catalog, OAuth, credentials, multi-account aliases, account states, and deletion | Direct with system-browser OAuth handoff | Unsupported initially | New connections preserve existing accounts; alias management, authorization return targets, active-only readiness, and credential redaction match Web |
