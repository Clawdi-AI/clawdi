...
| `src/shared/private-ip.ts` | Implemented | `private_ip.py` covers literal private ranges, private hostname aliases/suffixes, IPv4-mapped IPv6, CGNAT, private DNS results, and unresolved DNS fail-closed behavior; Telegram and BlueBubbles webhook registration use the shared guard. |
...
| `src/shared/webhook-delivery.ts` | Implemented | Provider route config replaces the old webhook table: Telegram `setWebhook/getWebhookInfo/deleteWebhook` and BlueBubbles webhook register/list/delete are persisted on channel accounts; `channel_webhooks.py` covers URL guards with delivery-time DNS revalidation, Telegram secret headers, delivery result metrics, Telegram single inline delivery attempt with worker redelivery, BlueBubbles retry semantics, and 4xx-as-failure behavior. |
| `src/shared/webhook-worker.ts` | Implemented | `ChannelWebhookDeliveryWorker` retries pending Telegram agent webhook inbox rows in order, SQL-skips non-webhook inbox rows, acks on success, preserves failed rows for retry, drops TTL-expired rows, and is run by `app.workers.channels` alongside outbound delivery. |
...
| `tests/integration/webhook-redelivery.test.ts` | Implemented | `test_channels.py` covers Telegram webhook single inline 5xx attempt, pending retention after repeated failures, worker redelivery/ack, TTL drop, and pair command control-message ack. |
...
| `tests/shared/private-ip.test.ts` | Implemented | `test_private_ip.py` ports the old literal/DNS matrix, including unresolved DNS fail-closed behavior, and route tests cover Telegram plus BlueBubbles DNS-private rejection. |
...
| `tests/shared/webhook-delivery.test.ts` | Implemented | Route-level tests cover set/get/delete style config, private URL rejection, Telegram secret-header delivery, Telegram single-attempt 5xx deferral, BlueBubbles 5xx retry, 4xx no-retry, and password query/header delivery behavior. |
| `tests/shared/webhook-worker.test.ts` | Implemented | `test_channels.py` covers ordered Telegram agent webhook redelivery through `ChannelWebhookDeliveryWorker`, success ack, 4xx/5xx failed attempt retention, non-webhook queue-head skipping, and TTL drop. |
