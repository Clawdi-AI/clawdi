...
| `tests/shared/webhook-delivery.test.ts` | Implemented | Route-level tests cover set/get/delete style config, private URL rejection, Telegram secret-header delivery, BlueBubbles 5xx retry, 4xx no-retry, and password query/header delivery behavior. |
