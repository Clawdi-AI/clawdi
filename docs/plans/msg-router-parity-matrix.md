...
| `src/core/pairing-flow.ts` | Implemented | Pair/unpair state machine is Clawdi-native in `resolve_inbound_binding`; one active binding is allowed per `(channel account, external chat)`, pairing can move that chat to a different bot-agent link, control commands are acked, and Discord interaction replies use old canonical msg-router text. |
