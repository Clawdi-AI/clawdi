...
Clawdi CLI. Engine-native config and Codex profile files are disposable
...
1. A user adds `openai-main` once and applies Codex and Hermes config from the
   same catalog. OpenClaw apply stays blocked until its native contract is
   pinned.
...
3. The happy path should be one provider add, one direct test, and one runtime
   apply.
4. Every command that could activate or patch engine-native config should support
...
- **Secret Ref**: a pointer to a secret source, for example
  `env:OPENAI_API_KEY`.
- **Projection**: an internal, engine-specific transformation from the Provider
  Catalog to the native config shape an agent engine can consume.
...
The user already configured providers in engine-native config and wants Clawdi
...
3. The apply plan includes the providers compatible with the target engine.
...
2. `ai-provider apply --dry-run` emits agent config that expects
   `OPENAI_API_KEY` or the engine's verified secret-ref equivalent.
...
be materialized through a verified native tool adapter until a target engine
...
`hermes config set`; OpenClaw v1 activation is blocked. The catalog remains
authoritative.
...
engine can consume the verified native secret-ref form directly.
...
5. **Engine projection adapters** generate OpenClaw, Hermes, or future engine
   config from normalized providers without mutating primary agent config.
...
7. **Import adapters** reverse existing engine-native config back into catalog
...
If a catalog contains providers that a target engine cannot represent,
...
machine, naming the provider IDs, protocols, and missing engine capabilities.
...
7. Do not embed drift markers into engine-native config files unless the
   pinned engine schema explicitly allows unknown metadata keys. Use external
...
Codex support is an engine projection adapter, not only credential import. It
...
5. **Validation failed**: schema, credential validation, or direct provider
   probe failed.
6. **Agent config apply failed**: CLI materialization reported an engine
...
3. Env vars referenced directly by engine-native config.
...
6. Agent apply should prefer managed config paths, eligible runtime CLI
   ...
   `hermes config set`; OpenClaw apply stays gated
...
4. **Dry-run/apply split vs direct config mutation**: the split is the right
   ...
   This avoids hidden edits to user-owned engine-native config.
