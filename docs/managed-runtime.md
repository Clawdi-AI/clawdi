...
| `egressProfiles` | Explicit local sidecar profiles plus any first-party Clawdi Agent Plugin profile derived by the backend from canonical desired state |
...
`/v1/agents/{agent_id}/agent-plugins/{plugin_name}` return accepted desired
state with convergence `not_observed`; list/get routes expose the same desired
rows. Product input is only the path plugin name and optional exact catalog
version. It cannot choose a URL, path, commit, digest, installation id, egress,
secret, or capability proof. A stable opaque installation UUID is generated by
the backend and remains stable across explicit updates. Catalog changes never
silently upgrade existing Agents.

This release installs only through the existing v2 Hosted bundle. An Agent
without Hosted v2 runtime state is rejected before persistence. Entries with
`ai.clawdi` configuration are listed as non-installable and rejected because
the current CLI accepts no plugin configuration; declared runtime compatibility
is also checked against the selected known runtime. There are no secret-slot
bindings in product input or runtime intent. The Platform compatibility input
keeps optional `agent_plugins` only as omitted/null; non-null input is rejected
and Platform no longer persists or assigns plugin selection.
...
public `X-Clawdi-Agent-Plugin: clawdi` routing marker. Clawdi derives an
egress profile server-side only for canonical trusted `clawdi` desired state.
It matches the fixed public package URL's exact
scheme, authority, path, and marker, rewrites it to the current environment's
canonical validated `public_api_url`, and sets `Authorization` from
...
The first-party identity is canonical plugin name `clawdi`, Agent Plugins
1.0.0 schema, trusted `https://github.com/Clawdi-AI/store`, and fixed
`v2/plugins/clawdi` path. Version, commit, and digest remain exact values from
the catalog-resolved immutable intent and are verified normally; they are not
authorization gates and Store updates require no CLI release. The opaque
`installationId` participates in proof ownership identity but not first-party
authorization. Any reserved name/path/schema mismatch or Hosted-supplied
reserved egress profile stops manifest rendering.
When desired state also contains other Agent Plugins, the unproven variant
projects only `clawdi` plus its capability probe and retains legacy
