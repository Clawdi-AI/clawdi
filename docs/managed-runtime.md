...
| Gateway health surfaces | [`server-http-probes.ts`](https://github.com/openclaw/openclaw/blob/8f382a202ff1e15833394b481615dcdda99b04d7/src/gateway/server-http-probes.ts), [`server-control-ui-root.ts`](https://github.com/openclaw/openclaw/blob/8f382a202ff1e15833394b481615dcdda99b04d7/src/gateway/server-control-ui-root.ts) | CLI apply completion proves required systemd units are active/enabled. The current Incus controller completes deployment from fresh, identity-matched CLI observation health. The [observer](../packages/cli/src/runtime/observed.ts) additionally requires the active OpenClaw gateway to return `ready: true` from loopback `/readyz` and HTTP 200 for its configured Control UI root, and the active Hermes dashboard to serve `/api/status` with `gateway_running: true`, `gateway_state: running`, and `auth_required` with provider `basic`. A failed or malformed probe leaves readiness unknown; image smoke probes alone do not gate deployed instances. |
