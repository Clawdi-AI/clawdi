...
| Gateway health surfaces | [`server-http-probes.ts`](https://github.com/openclaw/openclaw/blob/8f382a202ff1e15833394b481615dcdda99b04d7/src/gateway/server-http-probes.ts), [`server-control-ui-root.ts`](https://github.com/openclaw/openclaw/blob/8f382a202ff1e15833394b481615dcdda99b04d7/src/gateway/server-control-ui-root.ts) | CLI apply completion proves required systemd units are active/enabled. The [observer](../packages/cli/src/runtime/observed.ts) checks serving readiness asynchronously, using the applied receipt to retain required services even when their unit files are missing. It requires the active OpenClaw gateway to return `ready: true` from loopback `/readyz` and HTTP 200 for its native-configured port and Control UI root, and the active Hermes dashboard to serve `/api/status` with `gateway_running: true`, `gateway_state: running`, and `auth_required` with provider `basic`. A failed or malformed probe leaves readiness unknown. Readiness uses all observed units before diagnostic truncation and does not depend on the optional manifest cache. OpenClaw includes are resolved by its native `config get gateway --json` command. |
