...
| --- | --- | --- | --- | --- |
| Single socket and durable auth/Signal | Sidecar-owned only | No provider state | No provider state | Offline validation required |
| Text and replies | Typed, binding-scoped | Public send/receive contract exists | Public send contract exists | Live drill required |
| Media and files | Bounded typed content plus authorized opaque media id | Public outbound/media context exists | Base media methods exist | Live drill required |
| Reactions, typing, read receipts | Typed only where identity is unambiguous | Advertise only implemented public hooks | Advertise only implemented public hooks | Do not infer support |
| Edit and delete | Owned messages only | Advertise only implemented public hooks | Optional base methods exist | Live drill required |
| Inbox ACK/restart recovery | Backend ACK only after durable application handoff | Public durable ACK contract exists | Blocked: `handle_message()` returns after background scheduling ([`base.py`](https://github.com/NousResearch/hermes-agent/blob/f3cda0ceb18d8ba7465a6d223098ef0e56c8fee1/gateway/platforms/base.py#L5535-L5558)) | Hermes blocker |
