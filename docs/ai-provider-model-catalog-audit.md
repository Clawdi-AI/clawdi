...
- Baseline (merge-base with `origin/main`): `078fffd7`
...
- Kimi Code documents `k3` as supporting up to 1M context depending on
  membership, so that ID omits context metadata. The fixed `k3-256k` and
  `kimi-k3` IDs use the exact documented integer values; the rolling
  `kimi-for-coding` alias remains the all-member default and omits context.
