...
| `clawdi mcp http` | Start the compatibility MCP Streamable HTTP server for controlled runtimes |
...
| `clawdi mcp http --host <host> --port <port> --path <path> --auth-token-file <file>` | Expose explicit sidecar-local compatibility MCP |
| `clawdi runtime plan/apply/status --file <manifest>` | Preview, apply, and inspect local channel runtime manifest fixtures |
